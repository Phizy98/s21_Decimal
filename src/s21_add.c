...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  int res = 0;
  s21_decimal res_first = {0};
  s21_decimal res_second = {0};
...
// Проверяет, нет ли переполнения до того, как нормализация сведет к нулю
// маленькое число Возвращает 0 - если все хорошо, 1 - переполнение в сторону
// положительной бесконечности, 2 - переполнение в сторону отрицательной
// бесконечности
...
// Если экспонента больше 28 у положительного децимала, возвращается 1, если у
// отрицательного - 2, если экспоента корректна - 0
...
// Для чисел, не вмещающихся в децимал. Производит банковское округление по
// последним цифрам. Возвращает 1 в случае переполнения мантиссы, 0 - если все
// хорошо
int s21_banking_round_for_add(int remainder_1[], int remainder_2[],
  ...
  // for (int i = 0; i < 30; i++) printf("%d ", remainder_1[i]);
  // printf("\n");
