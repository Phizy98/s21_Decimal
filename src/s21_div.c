...
int s21_div_big_ten_array(int value_1[], int value_2[], int result[]) {
  ...
  while (s21_is_zero_array_for_big_decimal(a) == 0 || counter != 0) {
    ...
    if (s21_compare_big_ten_array(a, b) == -1) {
      ...
      // size_of_ab++       это нужно сделать только один раз, хз как (и хз,
      // надо ли вообще)
    }
    // printf("EXP before while= %d\n", exp_result);
    // если до сих пор меньше, то qoutient = 0 записываем в ответ
    ...
    while (s21_compare_big_ten_array(a, b) == -1) {
      // printf("OOOOOOO\n");
      // if (exp_result > 89) break;

      // printf("first counter_of_null = %d\n", counter_of_null);
      //  quotient = 0;
      //  reverse_result[count_result] = quotient;
      //  count_result++;
      quotient = 1;
      // printf("EXP start while= %d\n", exp_result);
      // если дошли до конца и больше сносить нечего, то сносим 0, а
      // результирующую экспоненту увеличивает на 1
      if (counter == 0) {
        //                 if (s21_is_zero_array_for_big_decimal(a) == 1) {
        // //printf("HHH\n");
    ...
    // printf("Reverse_result: \n");
    // s21_print_big_ten_array(reverse_result);
  ...
  // printf("EXP_RESULT = %d\n", exp_result);
  // printf("Первые %d чисел в reverse_result - это мантиса\n", count_result -
  // exp_result);
